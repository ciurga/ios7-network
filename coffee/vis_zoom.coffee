...
Network = () ->
  ...
  network = (selection, data) ->
    ...
    vis = d3.select(selection).append("svg").attr("width", width).attr("height", height).attr("pointer-events", "all").attr("cursor","move")
    
    zoom = d3.behavior.zoom().translate([100, 50]).scale(.5).on("zoom", zoomed)
    vis = vis.append('svg')
      .call(zoom.on('zoom', zoom))
      .append('svg:g')
  ...
  filterNodes = (allNodes) ->
    ...
    if filter == "popular" or filter == "obscure"
      playcounts = allNodes.map((d) -> 1).sort(d3.ascending)
